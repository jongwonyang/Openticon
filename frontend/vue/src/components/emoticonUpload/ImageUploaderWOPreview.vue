...
<template>
  <div class="flex flex-col items-center justify-center aspect-square">
    <div @click="$refs.fileInput.click()" @drop="handleDrop" @dragover="handleDragOver" :class="[
      'w-full h-full bg-gray-100 rounded-md border-2 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-200','border-gray-400 border-dashed'
    ]">
      <span class="material-icons text-gray-400 text-6xl">add_circle_outline</span>
      <span class="text-gray-400 text-sm">이미지 불러오기</span>
      <input ref="fileInput" type="file" class="hidden" accept=".jpg,.jpeg,.png,.gif" multiple @change="handleFileInput" />
    </div>
  </div>  
</template>