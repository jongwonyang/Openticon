  ...
  >
    <div class="flex flex-row">
      ...
      <div class="flex flex-col justify-center items-start ml-4 flex-grow">
        <p class="text-lg text-center line-clamp-2 font-nnsqneo-bold">
          ...
          <span
            ...
            :class="
              emoticon.emoticonPack.public ? 'bg-green-500' : 'bg-red-500'
            "
            >{{ emoticon.emoticonPack.public ? "공개" : "비공개" }}</span
          >
        </p>
        <div>
          <span
      ...
      <div class="flex flex-col justify-center items-end pr-2">
        <div
          ...
          v-if="emoticon.type == 'EXAMINE'"
        >
          AI 검수 통과 실패
        </div>
        <div
          class="text-red-500 font-nnsqneo-bold"
          v-if="emoticon.type == 'REPORT'"
        >
          신고 다수 발생
        </div>
      </div>
    ...
    <Transition name="fade">
      ...
      >
        ...
        <EmoticonImageGrid
          :emoticons="emoticon.emoticonPack.emoticons"
          :itemsPerPage="12"
        />
        <UserAction :emoticon="emoticon" />
      </div>
    </Transition>
  </div>
</template>
